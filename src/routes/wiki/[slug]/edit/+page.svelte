...
const editKey = $derived(
  data.isNew ? (data.slug === 'new' ? 'new' : data.slug) : (data.page?.slug ?? '')
)
...
async function handleInfoboxImageUpload(files: FileList | null, entryIndex: number) {
  ...
  try {
    ...
    const entries = infoboxData.entries.map((existing, index) =>
      index === entryIndex && existing.type === 'image' ? { ...existing, image: url } : existing
    )
...
<svelte:head>
  <title
    >{data.isNew ? data.defaultTitle || 'New page' : `Editing: ${data.page?.title}`} — Wiki</title
  >
</svelte:head>
...
<div class="flex flex-col h-[calc(100vh-3.5rem)]">
  <WikiEditHeader bind:title bind:namespace {showPreview} onTogglePreview={togglePreview} />
