...
const alphabetical = $derived([...data.pages].sort((a, b) => a.title.localeCompare(b.title)))
  ...
  {#if data.pages.length === 0}
    <div class="text-center py-16">
      ...
      <p class="text-base-content/50 text-sm">
        No {data.ns === 'template' ? 'templates' : 'pages'} yet.
      </p>
    </div>
  {:else}
    <!-- Alphabetical groups -->
          ...
          <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {#each pages as page}
              ...
              >
                ...
                <ArrowRight
                  size={13}
                  class="shrink-0 text-base-content/30 group-hover:text-primary transition-colors ml-2"
                />
              </a>
            {/each}
          </div>
