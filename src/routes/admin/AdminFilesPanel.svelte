  ...
  <table class="w-full text-sm">
    <thead>
      <tr class="bg-base-200/60 border-b border-base-200">
        <th class="text-left px-4 py-2.5 font-semibold text-base-content/60 text-xs uppercase tracking-wider">File</th>
        <th class="text-left px-3 py-2.5 font-semibold text-base-content/60 text-xs uppercase tracking-wider">Type</th>
        <th class="text-left px-3 py-2.5 font-semibold text-base-content/60 text-xs uppercase tracking-wider">Size</th>
        <th class="text-left px-3 py-2.5 font-semibold text-base-content/60 text-xs uppercase tracking-wider">Uploaded</th>
    ...
    <tbody class="divide-y divide-base-200">
      {#each filtered as file}
        <tr class="hover:bg-base-200/30 transition-colors group">
          ...
          <td class="px-3 py-3 text-base-content/50 text-xs">
            <time datetime={toDatetimeAttr(file.created_at)} title={formatDateTime(file.created_at)}>
          ...
          <td class="px-4 py-3">
            <div class="flex gap-1 justify-end opacity-100 lg:opacity-70 lg:group-hover:opacity-100 lg:group-focus-within:opacity-100">
              ...
              <button onclick={() => openRename(file.filename)}
                class="w-7 h-7 flex items-center justify-center rounded-md hover:bg-base-200 text-base-content/50 hover:text-base-content transition-all"
      ...
      {#if filtered.length === 0}
        <tr><td colspan="5" class="text-center py-12 text-base-content/30 text-sm">No files uploaded yet.</td></tr>
      {/if}
    </tbody>
  </table>
</div>
