  ...
  <table class="w-full text-sm">
    <thead>
      <tr class="bg-base-200/60 border-b border-base-200">
        <th class="text-left px-4 py-2.5 font-semibold text-base-content/60 text-xs uppercase tracking-wider">Title</th>
        <th class="text-left px-3 py-2.5 font-semibold text-base-content/60 text-xs uppercase tracking-wider">Namespace</th>
    ...
    <tbody class="divide-y divide-base-200">
      {#each pages as page}
        <tr class="hover:bg-base-200/30 transition-colors group">
          <td class="px-4 py-3">
            <a href="/wiki/{page.slug}" class="font-medium text-base-content hover:text-primary transition-colors">
              {page.title}
            </a>
      ...
      {/each}
      {#if pages.length === 0}
        <tr><td colspan="4" class="text-center py-12 text-base-content/30 text-sm">No pages found</td></tr>
      {/if}
    </tbody>
  </table>
</div>
