...
describe('seed admin user', () => {
  it('creates admin with configured seed password and must_change_pw on first open', async () => {
    const user = getUserByName('admin')
    expect(user).not.toBeNull()
    expect(user?.must_change_pw).toBe(1)
    expect(user?.is_admin).toBe(1)
    expect(await verifyPasswordAsync(TEST_ADMIN_PASSWORD, user!.password_hash)).toBe(true)
  })
  ...
  it('does not reset admin password when the database is reopened', async () => {
    ...
    expect(await verifyPasswordAsync(TEST_ADMIN_PASSWORD, reopened!.password_hash)).toBe(false)
  })
})
