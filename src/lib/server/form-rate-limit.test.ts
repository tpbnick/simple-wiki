...
import { enforceFormWriteRateLimit, formWriteRateLimitMessage } from '$lib/server/form-rate-limit.js'
...
describe('formWriteRateLimitMessage', () => {
  it('returns null when the caller is not authenticated', () => {
    expect(
      formWriteRateLimitMessage({} as App.Locals, () => '127.0.0.1', 'pages-save')
    ).toBeNull()
  })

  it('returns null when under the limit', () => {
    expect(
      formWriteRateLimitMessage({ user }, () => '127.0.0.1', 'pages-save-test')
    ).toBeNull()
  })
  ...
  it('honors custom limit and window options', () => {
    ...
    expect(
      formWriteRateLimitMessage({ user }, () => address, 'custom-limit', 1, 60_000)
    ).toBeNull()
    expect(
      formWriteRateLimitMessage({ user }, () => address, 'custom-limit', 1, 60_000)
    ).toMatch(/Too many requests/)
...
describe('enforceFormWriteRateLimit', () => {
  it('returns fail(429) with the configured field and tab', () => {
    ...
    const result = enforceFormWriteRateLimit(
      { user },
      () => address,
      'form-save-blocked',
      { field: 'userError', tab: 'users' }
    )
