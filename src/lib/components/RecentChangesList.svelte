...
{#if changes.length === 0}
  <div class="text-center py-16">
    ...
    <p class="text-base-content/35 text-xs">Edits appear here after a page is saved with changes.</p>
  </div>
{:else}
  <div class="space-y-8">
    {#each grouped as [day, dayChanges]}
      <div>
        <div class="flex items-center gap-3 mb-3">
          <span class="text-xs font-semibold uppercase tracking-wider text-base-content/40 whitespace-nowrap">{day}</span>
                  ...
                  >
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="flex flex-wrap items-center gap-x-2 gap-y-0.5">
                        <span class="font-medium text-sm text-base-content group-hover:text-primary transition-colors truncate">
                          {change.title}
                        </span>
                        {#if change.namespace !== 'article'}
                          <span class="px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wide bg-base-200 text-base-content/50 font-medium">
                            {change.namespace}
                          </span>
                        {/if}
                      </div>
                ...
                <div class="flex items-center gap-1 shrink-0 ml-2">
                  ...
                  {#if expandable}
                    ...
                    >
                      <ChevronDown
                        size={13}
                        class="transition-transform duration-200 {expandedId === change.id ? 'rotate-180' : ''}"
                      />
                    </button>
                  {/if}
                </div>
