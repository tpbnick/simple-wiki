<script lang="ts">
  ...
  function patch(fields: Partial<Pick<FamilyTreePerson, 'name' | 'birthYear' | 'deathYear' | 'imageUrl'>>) {
...
{#if person}
  <aside class="ft-editor">
    ...
    <label class="ft-field">
      ...
      />
      <span class="text-[0.7rem] text-base-content/45">Use [[Page Name]] to link to a wiki page.</span>
    </label>
    ...
    <div class="ft-field">
      <span>Photo</span>
      <input bind:this={fileInput} type="file" accept="image/*" class="hidden" onchange={handleImageUpload} />
      <button type="button" class="btn btn-sm btn-outline w-full" disabled={uploading} onclick={() => fileInput?.click()}>
        <Upload size={14} />
    ...
    {#if person.parentIds.length < 2}
      {#if person.parentIds.length === 0}
        <p class="ft-editor__notice">No parent is linked. Reconnect to someone already in the tree, or add a new parent.</p>
      {/if}
      {#if linkableParents.length > 0}
        <label class="ft-field">
          <span>{person.parentIds.length === 0 ? 'Reconnect to parent' : 'Link existing parent'}</span>
          <select
    ...
    <div class="ft-editor__actions">
      {#if person.parentIds.length < 2}
        <button type="button" class="btn btn-sm btn-outline" onclick={() => onchange(addParent(data, personId))}>
          <ArrowUpFromLine size={14} />
          Add parent
        </button>
      {/if}
      {#if !person.spouseId}
        <button type="button" class="btn btn-sm btn-outline" onclick={() => onchange(addSpouse(data, personId))}>
          <UserPlus size={14} />
          Add spouse
        </button>
      {/if}
      <button type="button" class="btn btn-sm btn-outline" onclick={() => onchange(addChild(data, personId))}>
        <Baby size={14} />
        Add child
      </button>
